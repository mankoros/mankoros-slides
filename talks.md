...
为了尽可能地提高我们内核的异步程度, 我们基本上实现了所有能异步实现的地方. 首先是异步的管道, 只有在读取端读取了管道内容之后, 写入端才会被唤醒执行, 反之亦然, 使得管道两端自然地根据数据产生速率得到了与之匹配的调度时间. 其次是内核中的异步睡眠队列.
TODO
...
Cemu 是一个类似 qemu 的模拟器, 它比 qemu 简单, 但仍然可以模拟运行 linux 内核. 
TODO
